...
def _resolves_to_metadata_endpoint(monkeypatch):
    """Make every hostname resolve to the cloud metadata address."""
    monkeypatch.setattr(
        http_exec_module.socket,
        "getaddrinfo",
        lambda _host, _port: [(2, 1, 6, "", ("169.254.169.254", 0))],
    )


def test_mcp_call_is_blocked_by_the_ssrf_guard(monkeypatch):
    from app.services.tools.mcp_client import call_mcp_tool

    _resolves_to_metadata_endpoint(monkeypatch)
    row = AgentTool(type="mcp", name="internal", url="https://mcp.example.test/mcp", transport="streamable_http")
    result, is_error = asyncio.run(call_mcp_tool(row, "anything", {}))
    assert is_error and "private or reserved" in result


def test_creating_an_mcp_tool_with_a_private_url_is_rejected(authenticated_client: TestClient, monkeypatch):
    """The guard runs during discovery, so an internal server never gets saved."""
    client = authenticated_client
    agent_id = _setup_agent(client)
    _resolves_to_metadata_endpoint(monkeypatch)
    rejected = client.post(
        f"/api/agents/{agent_id}/tools",
        json={"type": "mcp", "name": "metadata", "url": "https://mcp.example.test/mcp"},
    )
    assert rejected.status_code == 422
    assert "private or reserved" in rejected.json()["detail"]
    assert client.get(f"/api/agents/{agent_id}/tools").json() == []
