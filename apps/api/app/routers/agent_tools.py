...
from ..services.tools.mcp_client import BlockedUrlError, _flatten_exceptions, describe_mcp_error, discover_mcp_tools
...
async def _discover_or_502(url: str, transport: str, headers: dict[str, str] | None) -> list[dict]:
    try:
        ...
        # The URL was rejected before any connection: that is a bad request, not
        # an unreachable gateway.
        raise HTTPException(
            status_code=422,
            detail=f"The MCP server URL was rejected: {str(exc).removeprefix('Error: ')}.",
        ) from exc
