...
def _parse_message(message: dict, contacts: dict[str, str]) -> InboundMessage | None:
    ...
    if kind == "interactive":
        # A tapped reply button or list row. The id is ours (we set it when the
        # menu was sent); the title is what the contact saw, and it is what gets
        # stored so the thread reads like a normal message.
        interactive = message.get("interactive") or {}
        reply = interactive.get("button_reply") or interactive.get("list_reply") or {}
        if not reply.get("id"):
            return None
        return InboundMessage(**base, text=reply.get("title") or "", interactive_payload=reply.get("id"))
    if kind == "button":
        # Quick-reply button on a template message: same idea, different shape.
        button = message.get("button") or {}
        return InboundMessage(
            **base, text=button.get("text") or "", interactive_payload=button.get("payload") or None
        )
