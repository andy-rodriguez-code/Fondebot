...
def _validate_department(db: Session, client: Client, department_id: uuid.UUID | None) -> None:
    if department_id is None:
        return
    if not db.scalar(
        select(Department.id).where(Department.id == department_id, Department.client_id == client.id)
    ):
        raise HTTPException(status_code=422, detail="That department does not belong to this client")
...
def _portal_user_out(db: Session, portal_user: PortalUser) -> PortalUserOut:
    ...
    return PortalUserOut(
        ...
        department_id=portal_user.department_id,
        department_name=portal_user.department.name if portal_user.department else None,
...
def create_portal_user(
    ...
    _validate_department(db, client, payload.department_id)
...
def update_portal_user(
    ...
    if "department_id" in values:
        _validate_department(db, portal_user.client, values["department_id"])
        # Este es el único campo que se puede vaciar a propósito: sacar a la
        # persona de su dependencia la deja viendo todo el cliente. El bucle de
        # abajo asigna todos los valores, incluido None, así que no hace falta
        # tratarlo aparte.
