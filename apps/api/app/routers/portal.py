...
def _visible(user: PortalUser | None):
    """Restrict a conversation query to what this person may see.

    Someone who belongs to a department only ever sees that department's cases:
    treasury must not read what accounting is handling, and pasting a
    conversation id must not get around it. A person with no department sees
    the whole client, which is what supervisors need and what every account
    created before departments existed keeps doing.
    """
...
def _detail(db: Session, client: Client, conversation_id: uuid.UUID, user: PortalUser | None) -> Conversation:
    conversation = db.scalar(
        select(Conversation)
        .options(selectinload(Conversation.messages).selectinload(Message.attachments), joinedload(Conversation.agent), joinedload(Conversation.assignee), joinedload(Conversation.department))
        .execution_options(populate_existing=True)
        .where(Conversation.id == conversation_id, Conversation.client_id == client.id, _visible(user))
    )
    if not conversation:
        # 404 and not 403 on purpose: whether a conversation exists in another
...
def portal_members(
    ...
    Someone in a department can only hand a case to their own colleagues: they
    only see that department's conversations, so offering names from another
    one would just make the case disappear from their inbox.
    """
    query = select(PortalUser).where(PortalUser.client_id == client.id, PortalUser.is_active.is_(True))
    if user and user.department_id:
...
def portal_conversations(
    ...
    query = (
        select(Conversation, last.c.content, unread_count.label("unread_count"), PortalUser.name.label("assignee_name"), PortalUser.email.label("assignee_email"), last_inbound.c.at.label("last_inbound_at"), Department.name.label("department_name"))
        ...
        .outerjoin(Department, Department.id == Conversation.department_id)
        .where(Conversation.client_id == client.id, _visible(user))
    )
    ...
    return [
        ...
        for conv, content, row_unread_count, assignee_name, assignee_email, last_inbound_at, department_name in rows
    ]
...
def portal_mark_read(
    ...
    conversation = db.scalar(
        select(Conversation).where(
            Conversation.id == conversation_id, Conversation.client_id == client.id, _visible(user)
        )
    )
    if not conversation:
        raise HTTPException(status_code=404, detail="Conversation not found")
...
def portal_contact_conversations(
    ...
    rows = db.execute(
        ...
        .outerjoin(last, last.c.cid == Conversation.id)
        .where(Conversation.contact_id == contact.id, _visible(user))
...
async def portal_reply_template(
    ...
    conversation = _detail(db, client, conversation_id, user)
...
async def portal_assign(
    ...
    candidates = select(PortalUser).where(
        PortalUser.id == payload.assignee_id, PortalUser.client_id == client.id, PortalUser.is_active.is_(True)
    )
    if user and user.department_id:
        # Same reason as the members list: handing a case outside your own
        # department only makes it vanish from the inbox that can act on it.
        candidates = candidates.where(PortalUser.department_id == user.department_id)
    assignee = db.scalar(candidates)
    if not assignee:
