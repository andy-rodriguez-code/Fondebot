...
# Interactive message limits. Meta rejects the whole message when any of these
# is exceeded, so callers truncate rather than risk a 400 on a menu.
MAX_BUTTONS = 3
MAX_LIST_ROWS = 10
BUTTON_TITLE_CHARS = 20
LIST_TITLE_CHARS = 24
LIST_DESCRIPTION_CHARS = 72
LIST_BUTTON_CHARS = 20
INTERACTIVE_BODY_CHARS = 1024
...
async def _send_interactive(access_token: str, phone_number_id: str, to: str, interactive: dict) -> str | None:
    payload = {
        "messaging_product": "whatsapp",
        "to": to,
        "type": "interactive",
...
async def send_buttons(
    access_token: str, phone_number_id: str, to: str, body: str, options: list[tuple[str, str]]
) -> str | None:
    """Send up to three reply buttons; returns the outbound message id.

    ``options`` are ``(payload_id, title)`` pairs. The id comes back verbatim in
    the webhook as ``interactive.button_reply.id``, which is what lets a tap be
    matched to what it meant without parsing the visible title.
    """
    buttons = [
        {"type": "reply", "reply": {"id": option_id, "title": title[:BUTTON_TITLE_CHARS]}}
        for option_id, title in options[:MAX_BUTTONS]
    ]
    return await _send_interactive(
        access_token,
        phone_number_id,
        to,
        {
            "type": "button",
            "body": {"text": body[:INTERACTIVE_BODY_CHARS]},
...
async def send_list(
    ...
    """Send a single-section list menu (up to ten rows) for more options than
    buttons allow. ``options`` are ``(payload_id, title, description)``; the id
    returns as ``interactive.list_reply.id``."""
    rows = []
    for option_id, title, description in options[:MAX_LIST_ROWS]:
        row: dict = {"id": option_id, "title": title[:LIST_TITLE_CHARS]}
        if description:
            row["description"] = description[:LIST_DESCRIPTION_CHARS]
        rows.append(row)
    return await _send_interactive(
        access_token,
        phone_number_id,
        to,
        {
            ...
            "body": {"text": body[:INTERACTIVE_BODY_CHARS]},
            "action": {
                "button": button_label[:LIST_BUTTON_CHARS],
                "sections": [{"rows": rows}],
            },
        },
    )
