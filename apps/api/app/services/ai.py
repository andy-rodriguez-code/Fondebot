...
class Completion:
    ...
    input_tokens: int = 0
    output_tokens: int = 0
    # Lo define el loop de tools: [{name, arguments, result_preview, is_error}].
    tool_calls: list[dict] | None = None

# Fragmentos dentro del error 400 de un proveedor que significan que no acepta
# un parámetro de sampling (por ejemplo, modelos de razonamiento, o el límite de
# temperature <= 1 de Anthropic). Cuando los vemos, reintentamos una vez sin
# esos parámetros.
...
async def chat_completion(
    ...
    *,
    ...
    """Genera una respuesta usando la API de chat moderna del proveedor: la
    Responses API de OpenAI, o la Messages API de Anthropic."""
    try:
...
async def _anthropic_messages(base_url, api_key, model, messages, temperature, max_tokens) -> Completion:
    ...
    # Anthropic exige max_tokens; el sampling (temperature) se saca y se reintenta si da error.
...
def extract_openai_text(data: dict) -> str:
    """Saca el texto del asistente de un resultado de la Responses API de OpenAI."""
...
async def test_provider(provider: str, base_url: str, api_key: str) -> dict:
    """Verifica la clave de un proveedor listando sus modelos."""
    if provider == "anthropic":
