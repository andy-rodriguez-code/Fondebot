...
def devices_for_client(db: Session, client_id, *, department_id=None) -> list[Device]:
    ...
    ``department_id`` narrows the ring to the people who own that department's
    cases, plus anyone with no department at all — a supervisor sees everything,
    so their phone should ring for everything too. A device that belongs to no
    portal user is kept: it predates portal users and there is no one to
    attribute it to.
    """
    active = configured_provider()
    query = select(PushDevice).where(PushDevice.client_id == client_id)
    if department_id is not None:
        allowed = select(PortalUser.id).where(
            PortalUser.client_id == client_id,
            or_(PortalUser.department_id == department_id, PortalUser.department_id.is_(None)),
        )
        query = query.where(or_(PushDevice.portal_user_id.is_(None), PushDevice.portal_user_id.in_(allowed)))
...
async def notify_conversation(
    ...
    *,
    ...
    if not push_enabled():
        return 0
    devices = devices_for_client(db, conversation.client_id, department_id=conversation.department_id)
