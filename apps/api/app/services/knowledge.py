...
async def embed_document_chunks(db: Session, agent: Agent, document: KnowledgeDocument) -> int:
    """Trocea un documento ya procesado y guarda embeddings para la búsqueda semántica.

    Con la mejor intención posible: devuelve 0 (y no guarda nada) cuando el
    agente no tiene conexión o el proveedor no soporta embeddings; la búsqueda
    por palabras clave sigue funcionando igual.
    """
...
async def retrieve_knowledge(db: Session, agent: Agent, query: str) -> KnowledgeResult:
    """Devuelve el contexto de conocimiento para una consulta.

    Las bases de conocimiento chicas se incluyen enteras. Las más grandes usan
    búsqueda semántica sobre los embeddings guardados, y caen al ranking por
    palabras clave cuando los embeddings no están disponibles.
    """
    documents = list(
...
def _business_brief(agent: Agent) -> str:
    """Compone los campos completados del brief estructurado en un bloque etiquetado."""
    fields = (
