...
from .http_exec import _blocked_reason
...
class BlockedUrlError(ValueError):
    """The MCP server URL was rejected before any connection was attempted."""


@asynccontextmanager
async def open_mcp_session(url: str, transport: str, headers: dict[str, str] | None):
    # An MCP server URL is user-supplied and fetched server-side, exactly like an
    # HTTP tool URL, so it gets the same SSRF guard: without it any agency user
    # could point a tool at the internal network or the cloud metadata endpoint
    # and have the backend fetch it for them. Same opt-out
    # (TOOLS_ALLOW_PRIVATE_URLS) for self-hosted deployments that need it.
...
async def call_mcp_tool(tool: AgentTool, tool_name: str, args: dict) -> tuple[str, bool]:
    ...
    try:
        ...
        # The guard's message is already written for the LLM, and it says
        # something actionable, unlike the generic failure below.
        return str(exc), True
...
def describe_mcp_error(exc: BaseException) -> str:
    ...
    for cause in _flatten_exceptions(exc):
        if isinstance(cause, BlockedUrlError):
            return str(cause).removeprefix("Error: ")
