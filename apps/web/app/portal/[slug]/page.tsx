...
import { BadgeCheck, Bot, Building2, CheckCircle2, Clock, Contact as ContactIcon, FileText, FlaskConical, Globe, Images, Inbox, LayoutGrid, LoaderCircle, LogOut, MessageCircle, MessageSquareText, Search, Send, ShieldCheck, UserRound } from "lucide-react";
...
type Session = { client_id: string; client_name: string; portal_slug: string; agency_name: string; user_id?: string | null; user_name?: string | null; user_email?: string | null; department_name?: string | null; avatar_url?: string | null };
type Member = { id: string; name: string; email: string; avatar_url?: string | null; department_name?: string | null };
...
export default function PortalPage() {
  ...
  useEffect(() => { Promise.all([api<PortalPublic>(`/portal/${slug}`), api<Session>(`/portal/${slug}/me`).catch((err) => { if (err instanceof ApiError && err.status === 401) return null; throw err; })]).then(([info, me]) => { setPortal(info); setSession(me); }).catch((err) => { if (err instanceof ApiError && err.status === 403) { setSuspended(true); return; } setError(messageFrom(err)); }).finally(() => setLoading(false)); }, [slug]);
  ...
  return <PortalInbox slug={slug} portal={portal} session={session} logout={logout} onExpired={expire} onSession={setSession} />;
}

function PortalInbox({ slug, portal, session, logout, onExpired, onSession }: { slug: string; portal: PortalPublic; session: Session; logout: () => void; onExpired: () => void; onSession: (session: Session) => void }) {
  const t = useT();
  ...
  const [view, setView] = useState<"inbox" | "board" | "contacts" | "templates">("inbox");
  ...
  // El perfil vive en el padre (es la sesión), así que guardar acá tiene que
  // subir: si no, la barra sigue mostrando el nombre y la foto viejos.
  const onProfileSaved = useCallback((updated: Partial<Session>) => { onSession({ ...session, ...updated }); }, [onSession, session]);
  ...
  return <main className="portal-app" style={{ "--portal-color": portal.agency_brand_color } as React.CSSProperties}><aside className="portal-nav"><div className="portal-brand">{portal.client_logo_url || portal.agency_logo_url ? <img src={`${portal.client_logo_url || portal.agency_logo_url}`} alt="Logo" /> : <span>{portal.client_name.slice(0, 1)}</span>}<strong>{portal.client_name}</strong></div><nav><a className={view === "inbox" ? "active" : ""} onClick={() => setView("inbox")}><Inbox size={18} /> {t("portal.inbox.nav.inbox")}{summary && summary.unread > 0 && view !== "inbox" && <em className="nav-count">{summary.unread}</em>}</a><a className={view === "board" ? "active" : ""} onClick={() => setView("board")}><LayoutGrid size={18} /> {t("portal.inbox.nav.board")}</a><a className={view === "contacts" ? "active" : ""} onClick={() => setView("contacts")}><ContactIcon size={18} /> {t("portal.inbox.nav.contacts")}</a><a className={view === "templates" ? "active" : ""} onClick={() => setView("templates")}><FileText size={18} /> {t("portal.inbox.nav.templates")}</a><a className="disabled"><Bot size={18} /> {t("portal.inbox.nav.agents")}</a></nav><LanguageSwitcher /><button onClick={() => setProfileOpen(true)}><UserRound size={17} /> {t("portal.inbox.profile.open")}</button><button onClick={logout}><LogOut size={17} /> {t("portal.inbox.nav.logout")}</button></aside><section className="portal-main"><header><div><small>{t("portal.inbox.header.eyebrow")}</small><h1>{view === "contacts" ? t("portal.inbox.nav.contacts") : view === "templates" ? t("portal.inbox.nav.templates") : view === "board" ? t("portal.inbox.nav.board") : portal.portal_title}</h1></div>{view === "inbox" && <span>{t("portal.inbox.header.conversationsCount", { count: items.length })}</span>}</header>{view === "board" ? <BoardView slug={slug} myUserId={session.user_id ?? null} openConversation={openFromContact} /> : view === "templates" ? <TemplatesView slug={slug} supported={templatesSupported} /> : view === "contacts" ? <ContactsView slug={slug} channels={channels} openConversation={openFromContact} /> : items.length || query || tab !== "all" || status !== "open" ? <div className="portal-inbox"><aside onScroll={onListScroll}>
      ...
      {loadingMore && <div className="no-conversations"><LoaderCircle className="spin" size={16} /></div>}
              ...
              return <article key={message.id} className={`${message.role}${mine ? " mine" : ""}${mine && message.sender_type === "ai" ? " ai" : ""}${grouped ? " grouped" : ""}`}>
                {!grouped && <small className="msg-author">
                  {/* Quien contesto, de un vistazo: la cara de la persona, o el
                      icono del robot cuando contesto el agente. Un hilo que
                      solo muestra un nombre hace que las dos se vean iguales. */}
                  {mine && (message.sender_type === "ai"
                    ? <span className="msg-avatar bot" aria-hidden><Bot size={13} /></span>
                    : message.sender_avatar_url
                      ? <img className="msg-avatar" src={message.sender_avatar_url} alt="" loading="lazy" />
                      : <span className="msg-avatar initials" aria-hidden>{(message.sender_name || "?").slice(0, 1).toUpperCase()}</span>)}
                  <span>{message.sender_name || (message.role === "assistant" ? t("portal.inbox.conversation.agent") : t("portal.inbox.conversation.visitor"))}</span>
                  {message.sender_department && <em className="msg-department">{message.sender_department}</em>}
                </small>}
