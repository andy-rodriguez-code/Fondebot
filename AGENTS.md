...
- `app/logging_setup.py` — un identificador por pedido en cada línea de log, y en la cabecera `X-Request-Id` de la respuesta. Un `ContextVar` y no un global, porque tiene que ser distinto por pedido concurrente; eso además lo hace funcionar en los handlers `def` del threadpool, ya que `contextvars` viaja con el contexto. Es un **filtro** y no un formateador: un formateador que referencia un campo inexistente lanza, y los registros de una librería de terceros nunca lo traen. Un `X-Request-Id` entrante se respeta solo si pasa `[A-Za-z0-9._-]{1,64}` — sin eso, quien llama elige qué se escribe en el log y puede fabricar entradas con saltos de línea. `RequestIdMiddleware` se registra **después** de `ErrorCaptureMiddleware` para quedar por fuera de él. El id viaja al puente en `services/whatsapp.py` y sale en su log de fallas.
